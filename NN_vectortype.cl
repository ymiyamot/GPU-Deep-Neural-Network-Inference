             ...
             int weight_begin)
{
    ...
    for (int curr_tile_i = 0; curr_tile_i < total_tiles; curr_tile_i++) {
        ...
        for (int elem_i = 0; elem_i < lszx; elem_i++) {
            //if (wrkgrp_id_x == 0 & wrkgrp_id_y == 0) {
                acc += local_weights[ly * lszx + elem_i] * local_inputs[elem_i * lszx/vector_type+lx/vector_type];
            //}
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    
    outputs[gx/vector_type + n_inputs * gy] = acc;
