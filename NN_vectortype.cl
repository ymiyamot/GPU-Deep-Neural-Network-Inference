/// 4 versions of the vectorized kernel,
...
__kernel void
NN_gpu_vector2(__global float2 *inputs,
               __global __read_only float *weights,
               __global __write_only float2 *outputs,
               __local float2 *local_inputs,
               ...
               int weight_begin)
{
    ...
    for (int curr_tile_i = 0; curr_tile_i < total_tiles; curr_tile_i++) {
        if (idx_1D < lszx * vector_type) {
            ...
            //
            //                for (int i = 0; i < 16; i++) {
            //                    outputs[i][0] = local_weights[(2 * i)];
            //                    outputs[i][1] = local_weights[(2 * i) + 1];
            //                    outputs[i][2] = local_weights[(2 * i) + 2];
            //                    outputs[i][3] = local_weights[(2 * i) + 3];
            //                }
            //            }
        }
...
__kernel void
NN_gpu_vector4(__global float4 *inputs,
             __global __read_only float *weights,
...
__kernel void
NN_gpu_vector8(__global float8 *inputs,
               __global __read_only float *weights,
               ...
               int weight_begin)
{
    ...
    for (int curr_tile_i = 0; curr_tile_i < total_tiles; curr_tile_i++) {
        if (idx_1D < lszx * vector_type) {
            ...
            //
            //                for (int i = 0; i < 16; i++) {
            //                    outputs[i][0] = local_weights[(8 * i)];
            //                    outputs[i][1] = local_weights[(8 * i) + 1];
            //                    outputs[i][2] = local_weights[(8 * i) + 2];
            //                    outputs[i][3] = local_weights[(8 * i) + 3];
            //                }
            //            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);
...
__kernel void
NN_gpu_vector16(__global float16 *inputs,
               __global __read_only float *weights,
               __global __write_only float16 *outputs,
               __local float16 *local_inputs,
               __local float *local_weights,
               int n_prev,
               int n_inputs,
               int n_next,
               int weight_begin)
{
    ...
    for (int curr_tile_i = 0; curr_tile_i < total_tiles; curr_tile_i++) {
        if (idx_1D < lszx * vector_type) {
            ...
            //
            //                for (int i = 0; i < 16; i++) {
            //                    outputs[i][0] = local_weights[(16 * i)];
            //                    outputs[i][1] = local_weights[(16 * i) + 1];
            //                    outputs[i][2] = local_weights[(16 * i) + 2];
            //                    outputs[i][3] = local_weights[(16 * i) + 3];
            //                }
            //            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);
